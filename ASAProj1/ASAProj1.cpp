...
void ler() {
	...
	if (scanf("%f %d", &chapa_x, &chapa_y) != 2) { exit(1); }
	...
	vpa_da_gaveta = (float*)malloc((n_tipos + 1) * sizeof(int));
	n_tipos *= 2;
	todos_tipos = (tipo*)malloc(n_tipos * sizeof(tipo));
	for (i = 0; i < n_tipos; i++) {
		if (scanf("%d %d %f", &tipo_info.x, &tipo_info.y, &tipo_info.valor) != 3) { exit(1); }
...
/*bool por_nos_cantos(list<tipo> pecas_usadas, list<canto> cantos, canto canto_ant, int n_pos) {
	...
	if (!pecas_usadas.empty()) {
		...
		c1.lim_y = canto_ant.lim_y;
		c2.lim_x = canto_ant.lim_x;
		...
		for (//i = n_pos; i > 0; i--) {
			...
			if (canto_aux_pos->x < c1.x) {
				...
				canto_ant_pos++;
				if (c2.y < canto_aux_pos->y) {
					canto_ant_pos->lim_x = canto_aux_pos->x;
				}
				else {
					canto_ant_pos->lim_x = canto_aux_pos->lim_x;
				}
				canto_ant_pos--;
				break;
			}
		...
		canto_aux_pos = ++canto_ant_pos;
		for (i = n_pos + 2; i < (int)cantos.size(); i++) {
			canto_aux_pos++;
			if (canto_aux_pos->y < c2.y) {
				if (canto_aux_pos->x != c2.x) {
					canto_aux_pos->lim_x = c2.x;
				...
				canto_ant_pos--;
				if (c1.x < canto_aux_pos->x) {
					canto_ant_pos->lim_y = canto_aux_pos->y;
				}
				else{
					canto_ant_pos->lim_y = canto_aux_pos->lim_y;
				}
				break;
			}
		}
...
bool guilhotina(list<tipo> pecas_usadas, list<canto> cantos, canto canto_ant, int n_pos) {
	...
	if (!pecas_usadas.empty()) {
		canto c1, c2;
		bool c1_existe = false, c2_existe = false;
		int i;
		...
		if (canto_ant.x + peca.x != canto_ant.lim_x) {
			c1_existe = true;
			c1.lim_x = canto_ant.lim_x;
			c1 = criar_canto(canto_ant.x + peca.x, canto_ant.y);
		}
		if (canto_ant.y + peca.y != canto_ant.lim_y) {
			c2_existe = true;
			c2.lim_y = canto_ant.lim_y;
			c2 = criar_canto(canto_ant.x, canto_ant.y + peca.y);
		}
		if (c1_existe) {
			if (!c2_existe) {
				...
				n_pos = 0;
				for (canto c : cantos) {
					if (guilhotina(pecas_usadas, cantos, c, n_pos)) {
						return true;
					}
					n_pos++;
				}
...
int montagem(list<tipo> pecas_usadas, tipo nova_peca, int area_sobra_ant) {
	...
	if (!pecas_usadas.empty()) {
		...
		cantos.push_back(c);
		if (guilhotina(pecas_usadas, cantos, c, 0)) {
...
void cortar() {
	...
	tipo teste;
	int area_sobra, area_ant, valor_ant;
	bool first_time = true;
	int i, j;
	j = 0;
	area_sobra = chapa_area;
	...
	cortar_substituir(tipos_usados, area_ant, valor_ant);
